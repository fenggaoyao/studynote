  ...
  <label class="el-switch" :class="{ 'is-disabled': disabled, 'el-switch--wide': hasText }">
    ...
    <transition name="label-fade">
      <div
        ...
        <span v-if="!onIconClass && textOn">{{ textOn }}</span>
      </div>
    </transition>
    <transition name="label-fade">
      <div
        ...
        <span v-if="!offIconClass && textOff">{{ textOff }}</span>
      </div>
    </transition>
  </label>
</template>
...
<script>
  export default {
    ...
    methods: {
      ...
      setBackgroundColor() {
        let newColor = this.value ? this.colorOn : this.colorOff;
    ...
    mounted() {
      ...
      this.handleButtonTransform();
      if ((this.colorOn || this.colorOff) && !this.disabled) {
        this.setBackgroundColor();
      }
    }
  };
